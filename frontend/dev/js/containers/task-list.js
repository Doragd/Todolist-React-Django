...
class TaskList extends Component {
    ...
    renderList() {
        return this.props.tasks.map((task) => {
          if(!task.status){
            return (
              <tr key={task.id} onClick={() => this.props.selectTask(task)}><th>{task.title}</th><th>{task.expire_date}</th><th>{task.priority}</th><th>Doing</th></tr>
            );
          }
          else{
            return (
              <tr key={task.id} onClick={() => this.props.selectTask(task)}><th>{task.title}</th><th>{task.expire_date}</th><th>{task.priority}</th><th>Done</th></tr>
            );
          }
        });
    }
